...
async function sample1() {
  ...
  await analyzer.onBid(new BidWithSeat('S', 'normal', 1, 'H'));
  await analyzer.onBid(new BidWithSeat('W', 'pass'));
  await analyzer.onBid(new BidWithSeat('N', 'normal', 1, 'S'));
  await analyzer.onBid(new BidWithSeat('E', 'pass'));
  await analyzer.onBid(new BidWithSeat('S', 'normal', 2, 'S'));
  await analyzer.onBid(new BidWithSeat('W', 'pass'));
  await analyzer.onBid(new BidWithSeat('N', 'normal', 4, 'S'));
  await analyzer.onBid(new BidWithSeat('E', 'pass'));
